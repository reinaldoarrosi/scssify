...
function scssify(file, content, config, stream, done) {
  ...
  if (config._flags.debug) {
    ...
    sassOpts.sourceMap = sassOpts.sourceMap === undefined ? true : sassOpts.sourceMap;
    sassOpts.sourceMapEmbed = sassOpts.sourceMapEmbed === undefined ? true : sassOpts.sourceMapEmbed;
    sassOpts.sourceMapContents = sassOpts.sourceMapContents === undefined ? true : sassOpts.sourceMapContents;
    sassOpts.outputStyle = sassOpts.outputStyle === undefined ? 'nested' : sassOpts.outputStyle;
  }
  ...
  sass.render(sassOpts, function (err, firstResult) {
    if (err) done(new SyntaxError(err.formatted))
      ...
      postcss(postcssTransforms).process(firstResult.css, {
        ...
        to: file,
        map: {
          annotation: sassOpts.sourceMap ? true : false,
    ...
    function generateModule(result) {
      ...
      if (options.postProcess && typeof(options.postProcess) === 'function') {
        cssString = options.postProcess(cssString);
      }

      let out = 'var css = ' + cssString;
